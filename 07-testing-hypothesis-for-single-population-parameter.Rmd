...
-   [7.1 Testing Hypothesis for a Population
    Mean](../chapter07/0701.html)
-   [7.2 Testing Hypothesis for a Population
    Variance](../chapter07/0702.html)
-   [7.3 Testing Hypothesis for a Population
    Proportion](../chapter07/0703.html)
-   [7.4 Testing Hypothesis with α and β
    simultaneously](../chapter07/0704.html)
-   [  7.4.1 Type 2 Error and Power of a Test](../chapter07/0704.html)
-   [  7.4.2 Testing Hypothesis with α and β](../chapter07/0704.html)
-   [7.5 Exercise](../chapter07/0705.html)

##### [[\[book\]]{.underline}](book07.pdf){target="_blank"}    [[\[eStat YouTube Channel\]]{.underline}](https://www.youtube.com/channel/UCw2Rzl9A4rXMcT8ue8GH3IA){target="_blank"}
...
[presentation](0701.pdf){.presentation-link target="_blank"}   
...
rejection region of $\small H_0$ (\<Figure 7.1.1\>).
...
sample means in Chapter 6. \<Figure 7.1.2\> shows the distribution of
...
summarized in Table 7.1.2 when the population standard deviation is
...
In \[Example 7.1.1\], if the sample mean is either 1555 or 1540, the
...
location of the alternative hypothesis. In \[Example 7.1.1\], the
...
alternative hypothesis is summarized as in Table 7.1.3.
:::
...
testing hypothesis for the population mean can be done as Table 7.1.4
...
testing hypothesis in Table 7.1.4 can be performed using the standard
...
the right test in the window shown in \<Figure 7.1.3\>. Check \[Test
...
![](Figure/Fig070103.png){.imgFig600400}
...
::: textL20
If you click the \[Execute\] button, the confidence Interval for μ is
...
::: textL20
If you select the two-tail test at \[Hypothesis\] of \<Figure 7.1.3\>,
testing result using 『eStatU』 is as \<Figure 7.1.5\>.
:::
...
100 in \[Example 7.1.2\], test whether the average weight of the cookie
...
In \<Figure 7.1.3\> of 『eStatU』 , select the right-sided test of
\[Hypothesis\], select the $t$-test on \[Test Type\] and enter sample
size $n$ = 16, then the test result is as \<Figure 7.1.6\> if you click
the \[Execute\] button.
...
::: figText
\<Figure 7.1.6\> Testing hypothesis for μ with $t$ distribution using
『eStatU』
:::
...
After entering data on a sheet as shown in \<Figure 7.1.7\> in 『eStat』
...
\<Figure 7.1.9\>, the corresponding histogram is appeared as in \<Figure
...
display the hypothesis test graph as shown in \<Figure 7.1.11\> and a
test result in the Log Area as in \<Figure 7.1.12\>.
...
::: presentation-video-link
[presentation](0702.pdf){.presentation-link target="_blank"}   
...
significance level as \<Figure 7.2.1\> in the input box. Then enter the
...
::: figText
\<Figure 7.2.1\> Testing hypothesis for $\sigma^2$ using 『eStatU』
:::
...
::: textL20
If you click the \[Execute\] button, the confidence interval of
...
After entering data as shown in \<Figure 7.2.3\> on the sheet in
...
(average) ± (standard deviation) interval as in\<Figure 7.2.4\>.
...
::: figText
\<Figure 7.2.4\> Dot graph and (Mean) ± (Std Dev) for Testing hypothesis
of $σ^2$
:::

In the option box under the Graph Area (\<Figure 7.2.5\>), enter
...
\<Figure 7.2.6\> and the result table as \<Figure 7.2.7\>.
...
distribution which can be drawn from the option box in \<Figure 7.2.5\>.
...
::: presentation-video-link
[presentation](0703.pdf){.presentation-link target="_blank"}   
...
population mean as Table 7.3.1. If $np$ \> 5 and $n(1-p)$ \> 5, it is
...
\<Figure 7.3.1\> in the input box window. Then enter the sample size $n$
= 100, and the sample proportion $\hat p = 0.55$. If you click the
\[Execute\] button, the confidence interval of $p$ is calculated and
testing result will be shown as in \<Figure 7.3.2\>.
:::
...
::: figText
\<Figure 7.3.1\> Data input for testing hypothesis of $p$ using
『eStatU』
:::
...
::: presentation-video-link
[presentation](0704.pdf){.presentation-link target="_blank"}   
...
in the input box window as \<Figure 7.4.1\> and click the \[Execute\]
...
::: mainTableGrey
**Example 7.4.2** In \[Example 7.1.1\], if the null hypothesis is not
...
\<Figure 7.4.1\> and click the \[Execute\] button.
:::
...
::: mainTable
Comparing \[Example 7.4.1\] and \[Example 7.4.2\], the probability of
...
::: mainTableGrey
**Example 7.4.3** In \[Example 7.1.1\], calculate the power of the
...
input box window as \<Figure 7.4.4\>.
...
If you click the \[Execute\] button, you will see the test result of
『eStatU』 as in \<Figure 7.4.5\>. The critical value and the sample
size are calculated.
